    ...
    <Layout class="layout">
        ...
        <Col>
          <div>
            <Breadcrumb :style="{margin: '20px 15px 0px 15px'}">
              ...
              <BreadcrumbItem>开展核查</BreadcrumbItem>
            </Breadcrumb>
          </div>
        </Col>
      </Row>
      <Form :label-width="120" :model="person" ref="Form">
        <Row>
          ...
          <Form-item label="开展原因" required>
            <Input v-model="reason" type="textarea" :rows="2" placeholder="请输入开展核查的原因" style="width: 600px"></Input>
          </Form-item>
          </Col>
          <Col span="12">
    ...
    methods: {
      ...
      goSave () {
        ...
        axios.get(API.Active, {
          ...
          if (res.data === 'OK') {
            this.$Loading.finish()
            this.$Message.success('开展核查成功!')
            this.$Notice.success({
              title: '操作完成!',
              desc: '家属：' + this.name + '已开展核查！'
            })
            setTimeout(() => {
